    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            scale_factor = np.array([w_scale, h_scale, w_scale, h_scale],
                                    dtype=np.float32)
        # Added by lzh
        # For cases in which img.shape=(512, 512, 1)
        # after cv2.resize(im, size), img.shape=(800, 800)
        if len(img.shape)==2 and len(results['img'].shape)==3:
...
class Normalize(object):
    ...
    def __call__(self, results):
        if self.is_3d_input:
            mean_3d = tuple([self.mean[0] for i in range(self.num_slice)])
            std_3d = tuple([self.std[0] for i in range(self.num_slice)])
            self.mean, self.std = mean_3d, std_3d
            self.to_rgb = False
            results['img'] = mmcv.imnormalize(results['img'], mean_3d, std_3d, self.to_rgb)
        else:
