...
class ImageTransform(object):
    ...
    def __call__(self, img, scale, flip=False, keep_ratio=True, num_slice=3, is_3d_input=False):
        if keep_ratio:
        ...
        img_shape = img.shape
        if is_3d_input:
            # for 3d input , len(self.mean) should be equal to num_slice
            mean_3d =tuple([self.mean[0] for i in range(num_slice)])
            std_3d = tuple([self.std[0] for i in range(num_slice)])
            # for 3d input ，to_rgb set false
            img = mmcv.imnormalize(img, mean_3d, std_3d, False)
        else:
