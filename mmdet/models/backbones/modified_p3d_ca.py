...
from .cross_layer_fusion import CSABlock, deprocess_image

__all__ = ['modified_P3D_CA', 'modified_P3D18', 'modified_P3D63', 'modified_P3D131','modified_P3D199']
...
class modified_P3D_CA(nn.Module):
    """ResNet backbone.
    ...
    def __init__(self,
                 ...
                 dropout=0.5,
                 ST_struc=('A','B','C'),
                 cross_slice_attention_type='maxpool',
                 conversion_type='center_crop',
                 conversion_slice_num=9):
        super(modified_P3D_CA, self).__init__()
        ...
        self.layer1 = self._make_layer(Bottleneck, 64, self.stage_blocks[0], shortcut_type)
        self.layer2 = self._make_layer(Bottleneck, 128, self.stage_blocks[1], shortcut_type, stride=2)
        self.layer3 = self._make_layer(Bottleneck, 256, self.stage_blocks[2], shortcut_type, stride=2)
        self.layer4 = self._make_layer(Bottleneck, 512, self.stage_blocks[3], shortcut_type, stride=2)
        # Cross Slice Attention Blocks
        self.attention_layers = nn.ModuleList()
        #if self.cross_layer_fusion:
            #for in_channels in [256, 512, 1024, 2048]:
                #self.attention_layers.append(CSABlock(in_channels, in_channels // 2))

        self.conversion_dict = {'center_crop': self.center_cropping}
        if self.conversion_type=='conv3d':
            self.conv3d_all_1 = nn.Conv3d(256, 256, kernel_size=(self.conversion_slice_num, 1, 1), stride=(1, 1, 1),
                                          padding=(0, 0, 0), bias=False)
            self.conv3d_all_2 = nn.Conv3d(512, 512, kernel_size=(self.conversion_slice_num, 1, 1), stride=(1, 1, 1),
                                          padding=(0, 0, 0), bias=False)
            self.conv3d_all_3 = nn.Conv3d(1024, 1024, kernel_size=(self.conversion_slice_num, 1, 1), stride=(1, 1, 1),
                                          padding=(0, 0, 0), bias=False)
            self.conv3d_all_4 = nn.Conv3d(2048, 2048, kernel_size=(self.conversion_slice_num, 1, 1), stride=(1, 1, 1),
                                          padding=(0, 0, 0), bias=False)
            self.conv3d_all_dict = {256:self.conv3d_all_1, 512:self.conv3d_all_2, 1024:self.conv3d_all_3, 2048:self.conv3d_all_4}
            self.conversion_dict['conv3d'] = self.conv3d_all
        elif self.conversion_type=='groupconv':
            self.groupconv_1 = nn.Conv2d(256 * self.conversion_slice_num, 256, kernel_size=(1, 1), stride=(1, 1),
                                         padding=(0, 0), groups=256, bias=False)
            self.groupconv_2 = nn.Conv2d(512 * self.conversion_slice_num, 512, kernel_size=(1, 1), stride=(1, 1),
                                         padding=(0, 0), groups=512, bias=False)
            self.groupconv_3 = nn.Conv2d(1024 * self.conversion_slice_num, 1024, kernel_size=(1, 1), stride=(1, 1),
                                         padding=(0, 0), groups=1024, bias=False)
            self.groupconv_4 = nn.Conv2d(2048 * self.conversion_slice_num, 2048, kernel_size=(1, 1), stride=(1, 1),
                                         padding=(0, 0), groups=2048, bias=False)
            self.group_conv_dict = {256:self.groupconv_1, 512:self.groupconv_2, 1024:self.groupconv_3, 2048:self.groupconv_4}
            self.conversion_dict['groupconv'] = self.group_conv

        elif self.conversion_type == 'cross_slice_attention':
            self.CSABlocks_1 = CSABlock(256, 128, cross_slice_attention_type)
            self.CSABlocks_2 = CSABlock(512, 256, cross_slice_attention_type)
            self.CSABlocks_3 = CSABlock(1024, 512, cross_slice_attention_type)
            self.CSABlocks_4 = CSABlock(2048,1024, cross_slice_attention_type)
            self.CSABlocks_dict = {256:self.CSABlocks_1, 512:self.CSABlocks_2,
                    1024:self.CSABlocks_3, 2048:self.CSABlocks_4}
            self.conversion_dict['cross_slice_attention'] = self.cross_slice_attention_block
        elif self.conversion_type == 'nonlocal2d':
            self.NonLocal2D_1 = NonLocal2D(256, use_scale=False, norm_cfg=self.norm_cfg)
            self.NonLocal2D_2 = NonLocal2D(512, use_scale=False, norm_cfg=self.norm_cfg)
            self.NonLocal2D_3 = NonLocal2D(1024, use_scale=False, norm_cfg=self.norm_cfg)
            self.NonLocal2D_4 = NonLocal2D(2048, use_scale=False, norm_cfg=self.norm_cfg)
            self.NonLocal2D_dict = {256:self.NonLocal2D_1, 512:self.NonLocal2D_2,
                    1024:self.NonLocal2D_3, 2048:self.NonLocal2D_4}
            self.conversion_dict['nonlocal2d'] = self.nonlocal2d
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        n, c, d, h, w = x.shape
        #deprocess_image(x[:, :, d//2, :, :], 'before', 1)
        center_x = self.conversion_dict[self.conversion_type](x)
        #deprocess_image(center_x, 'after', 1)
        ...
        # x = self.layer4(x)
        # print('it is x layer4 ',x.shape)
        center_x = self.conversion_dict[self.conversion_type](x)
        outs.append(center_x)               # Part Res5
        return outs

    def train(self, mode=True):
        super(modified_P3D_CA, self).train(mode)
        self._freeze_stages()
    ...
    def cross_slice_attention_block(self, x):
        n, c, d, h, w = x.shape
        out = self.CSABlocks_dict[c](x)
        return out
    def nonlocal2d(self, x):
        n, c, d, h, w = x.shape
        x = x.view(n, c, d*h, w)
        if c == 256:
            out = x
        else:
            out = self.NonLocal2D_dict[c](x)
            #print(out.shape)
        out = out.view(n, c, d, h, w)
        return out[:, :, d//2, :, :]
