...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        # Added by lzh
        # data pre-process pipeline is complicated,
        # if we want to take a single CT image as input,
        # it's a convenient implementation to change the input \
        #   before sent into network.
        if self.data_mode == 'single_slice':
            n, c, h, w = x.shape
            x = x[:, c//2 : c//2+1, :, :]
        if self.data_mode == 'single_slice_as_rgb':
            n, c, h, w = x.shape
            x = x[:, (c//2, c//2,c//2), :, :]
        if len(x.shape) == 5:
            n, c, d, h, w = x.shape
    ...
    def train(self, mode=True):
        ...
        if mode and self.norm_eval:
            for m in self.modules():
                ...
                print(m, 'is this bn & to be fixed?: ', flag)
                if isinstance(m, _BatchNorm):
                    m.eval()

@BACKBONES.register_module
class ResNet_origin(ResNet):
    def __init__(self, **kwargs):
        super(ResNet_origin, self).__init__(**kwargs)
        self.maxpool = nn.MaxPool3d(kernel_size=3, stride=(1,2,2), padding=1)
        #print('init res2d, conv1', self.conv1)

    def forward(self, x):
        #print('forward', self.conv1)
        print(x.shape)
        x = self.conv1(x)
