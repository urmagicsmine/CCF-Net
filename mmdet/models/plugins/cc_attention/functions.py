...
import functools
import pdb
#from ..test_tools import show_statistic
...
class CrissCrossAttention(nn.Module):
    ...
    def __init__(self,in_dim):
        ...
        self.query_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim//4 , kernel_size=3, padding=1)
        self.key_conv = nn.Conv2d(in_channels = in_dim , out_channels = in_dim//4 , kernel_size=3, padding=1)
    ...
    def forward(self, x, ref=None):
        ...
        if type(ref)==type(None):
            ...
            energy = ca_weight(proj_query, proj_key) # shape(1, (h+w-1), h, w)
            attention = F.softmax(energy, 1)
            out = ca_map(attention, proj_value)
            out = self.gamma*out + x
        else:
            #print('ref branch')
            proj_query = self.query_conv(ref)
            proj_key = self.key_conv(x)
            proj_value = self.value_conv(x)
            ...
            out = ca_map(attention, proj_value)
            out = self.gamma*out + x
            #print(energy.shape, out.shape, x.shape)
        #show_statistic(energy, 'energy_y')
        #show_statistic(attention, 'attention')
...
if __name__ == "__main__":
    ...
    x = torch.zeros(1, 256, 10, 10).cuda() + 1
    y = torch.zeros(1, 256, 10, 10).cuda() + 2
