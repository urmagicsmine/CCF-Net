...
class AFNB(nn.Module):
    ...
    def __init__(self, low_in_channels, high_in_channels, out_channels, key_channels, value_channels, dropout,
        ...
        self.stages = nn.ModuleList(
            [self._make_stage([low_in_channels, high_in_channels], out_channels, key_channels, value_channels, size) for size in sizes]
            )
...
if __name__ == '__main__':
    ...
    y = torch.rand(1, 512, 4, 4)
    #afnb = AFNB(256, 512, 512, 64, 64, dropout=0.05, norm_type=None)
    afnb = SelfAttentionBlock2D(256, 512, 512, 64, 64, norm_type=None)
    out = afnb(x, y)
    print(out.shape)
